...
func (apple2_disk *Apple2Disk) FindFile(filename string) (int, int, bool) {
	...
	for i := 0; i < len(apple_name); i++ {
		apple_name[i] = 32 | 0x80
	}
	...
	for true {
		...
		for i := 0x0b; i < 256; i += 0x23 {
			...
			for n = 0; n < len(apple_name); n++ {
				...
				if apple2_disk.data[offset+i+n+3] != apple_name[n] {
					break
				}
			}
			...
			if n == len(apple_name) {
				is_binary := true
				if apple2_disk.data[offset+i+2]&0x7f == 0 {
					is_binary = false
				}
...
func (apple2_disk *Apple2Disk) PrintFileSectorList(track int, sector int) {
	...
	for true {
		...
		if apple2_disk.data[offset+0x01] == 0x00 {
			break
		}
...
func (apple2_disk *Apple2Disk) PrintTextFile(output_name string, track int, sector int) {
	...
	for true {
		...
		for i := 0x0c; i <= 0xff; i += 2 {
			if apple2_disk.data[offset+i] == 0 && apple2_disk.data[offset+i+1] == 0 {
				break
			}
	...
	for true {
		...
		for i := 0x0b; i < 256; i += 0x23 {
			...
			if apple2_disk.data[offset+i+0] == 0xff ||
				...
				if address != 0 {
					apple2_disk.data[offset+i+2] = 0x04
				}
				for n := 0; n < 30; n++ {
					apple2_disk.data[offset+i+3+n] = 0xa0
				}
